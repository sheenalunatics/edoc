 <?php
	...
	//if(validation_errors() != false) { echo iconv('TIS-620', 'UTF-8', 'ข้อมูลไม่ถูกต้อง'); }
	//foreach($query as $row){ 
?>
 <?=form_open('home/document_search_edit/'.$key_search, $attributes)?>
<section id="main"><article class="width_full"> 
 <header>
      <h3 class="tab_name"><?php echo iconv('TIS-620', 'UTF-8', 'ค้นหาเอกสาร');?></h3>
 </header>
	 ...
	 <table class="tableform">
		...
		<?php 
			...
			foreach($query as $row){ 
				...
				<td class="center"><!--[ <a href='#' onclick='ViewInfo("<?=$row['user_name']?>");'>View</a> ]-->[<a href='#' class="btn" onclick='EditInfo("<?=$row['doc_id']?>");'>Edit</a>]<!--[ <a href='#' onclick='DelInfo("<?=$row['user_name']?>");'>Delete</a> ]-->
				  </td>
			  </tr>
			<?php 
			} 			
...
$('.ui-autocomplete.ui-menu').css('fontSize', '12px');
 ...
 function EditInfo(id){
			window.location='<? echo base_url();?>'+'index.php/home/document_edit/'+id;
		}
</script>