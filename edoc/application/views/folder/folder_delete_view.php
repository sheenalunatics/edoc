...
?>
 ...
 <header>
      <h3 class="tab_name">DELETE FOLDER</h3>
 </header>
	<div  class="module_content">
		  <?=form_open('home/folder_delete', $attributes)?>
      <table class="tableform">
           ...
           <td class="center" width="10%">FolderID</td>
            ...
            <td class="center" width="10%">DivisionID</td>
			 <td class="center" width="20%">DateTime</td>
			   <td class="center" width="20%">CreateBy</td> 
		...
		if(!empty($query)){
			...
			<td class="center"><?=dateconvertTH($row['timestamp']);?></td>			
			...
			?>
			<a href='#' class="btn" onclick='DelInfo("<?=$row['folder_id']?>");'>Delete</a>
			<?php
				}
				?>
			]</td>
...
function DelInfo(id){
	if(confirm('<? echo iconv("TIS-620", "UTF-8", "ต้องการลบ?");?>')){
		window.location='<? echo base_url();?>'+'index.php/home/folder_delete/'+id;
	}
	else{
		return false;
	}	
